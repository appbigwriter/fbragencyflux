...
describe('FLUX-018 dashboard', () => {
 it('shows Info, four ordered linked metrics, four operational panels and Histórico with truthful empty states', () => {
  ...
  const panels = html.slice(html.indexOf('aria-label="Painéis operacionais"'), html.indexOf('aria-label="Histórico"'))
  expect(panels.indexOf('Projetos e cards prioritários')).toBeLessThan(panels.indexOf('Gates pendentes/recentes'))
  expect(panels.indexOf('Gates pendentes/recentes')).toBeLessThan(panels.indexOf('Central de Aprovações'))
  expect(panels.indexOf('Central de Aprovações')).toBeLessThan(panels.indexOf('Atenção necessária'))
  expect(panels).not.toContain('Atenção operacional')
 })
})
