...
const fields: Array<{ key: keyof ResolutionAction; label: string; required?: boolean }> = [
  { key: 'from', label: 'De', required: true }, { key: 'to', label: 'Destinatário / owner', required: true },
  ...
  { key: 'nextStep', label: 'Próximo passo', required: true }, { key: 'fallback', label: 'Fallback / ação requerida', required: true },
]
...
export default function BlockerAttention({ blockers, snapshot }: { blockers: Blocker[]; snapshot: DashboardSnapshot }) {
  ...
  function open(blocker: Blocker) { if (!session) { setMessage('Entre para passar este problema'); requestAuth('Entre para passar este problema'); return }; if (blocker.status !== 'open' || !blocker.resolutionAction) return; setSelected(blocker); setForm(defaults(blocker)); setError(null); setMessage('') }
  async function forward(event: FormEvent) {
    ...
    try {
      ...
      setMessage(body.idempotent ? 'Encaminhamento já registrado; nenhum evento duplicado foi criado.' : 'Encaminhamento persistido; blocker permanece aberto até evidência real.')
