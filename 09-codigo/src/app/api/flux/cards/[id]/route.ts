...
async function handle(request: Request, { params }: Params) { try { const session = requireRole(request, 'operator'); const body = await request.json() as { status?: CardStatus; reason?: string }; const { id } = await params; return NextResponse.json(await transitionCard(id, body.status as CardStatus, { actor: session.actor, scope: 'local' }, undefined, body.reason)) } catch (e) { return jsonError(e) } }
