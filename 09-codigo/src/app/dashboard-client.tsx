...
export default function DashboardClient({ initial }: { initial: DashboardSnapshot }) {
 ...
 return <main className={styles.shell}>
  ...
  <div className={styles.dashboardGrid} aria-label="Painéis operacionais">
   ...
   <section className={styles.panel}><div className={styles.panelHeader}><h2>Atenção operacional</h2></div><p><Link href="/handoffs?status=awaiting_owner">{awaiting} Handoffs aguardando owner</Link></p><p><Link href="/jobs?stale=true">{stale} jobs stale</Link></p><p><Link href="/?view=actions">{initial.requiredActions?.filter(a => a.status === 'hold').length || 0} ações em HOLD</Link></p>{jobs.filter(j => j.status !== 'completed').slice(0,3).map(j => <Link className={styles.compactItem} key={j.jobId} href={`/jobs?job=${encodeURIComponent(j.jobId)}`}><span><strong>{j.owner || j.agent}</strong><small>Próxima ação: {j.nextStep || 'não declarada'} · próximo check: {j.nextCheck || 'não declarado'}</small></span></Link>)}{initial.coordinator?.lastCoordinatorRun ? <Link className={styles.compactItem} href="/?view=coordinator">Último ciclo: {initial.coordinator.lastCoordinatorRun.completedAt}</Link> : <p className={styles.muted}>Nenhum ciclo de coordenação registrado.</p>}</section>
  </div>
