...
export type Event = { id: string; time: string; actor: string; action: string; cardId?: string; correlationId?: string; fromStatus?: string; toStatus?: string; reason?: string; blockerId?: string; jobId?: string; from?: string; to?: string; solution?: { cause: string; owner: string; nextAction: string; resolutionPlan: string; resolutionEvidence: string } }
...
export type JobStatus = 'planned' | 'in_progress' | 'review' | 'blocked' | 'awaiting_owner' | 'completed' | 'failed' | 'not_verified'
...
function deterministicHandoffId(sourcePath: string, index: number, text: string) { return `handoff-${createHash('sha256').update(`${sourcePath}\n${index}\n${text.trim()}`).digest('hex').slice(0, 20)}` }
function parseHistoricalHandoff(text: string, sourcePath: string, index: number): Handoff | null {
  const from = extractField(text, ['de']); const summary = extractField(text, ['objetivo(?: do job|_do_job)?', 'resumo']); const cardId = extractField(text, ['card'])
  if (!from || !summary || !cardId) return null
  const blockers = extractList(text, ['pend[êe]ncias(?:\\/|_)blockers', 'blockers']); const cause = blockerCause(text); const active = hasStructuredOpenBlocker(text)
  const parsedBlockers = (active && cause ? [cause, ...blockers] : blockers).map((cause, i) => ({ id: `blocker-${createHash('sha256').update(`${sourcePath}:${index}:${i}:${cause}`).digest('hex').slice(0, 16)}`, cause, status: active && i === 0 ? 'open' as const : 'legacy' as const }))
  const to = extractField(text, ['para', 'to']) || 'não declarado'; const now = new Date().toISOString()
  return { id: deterministicHandoffId(sourcePath, index, text), cardId, project: cardId.startsWith('AF-') ? 'After Forty' : 'FBR Agency Flux', from: from.split('/')[0].trim(), to, summary, done: extractField(text, ['done', 'conclu[ií]do', 'entreg[aá]vel', 'entregavel']), risks: extractField(text, ['riscos?', 'risks?']), nextStep: extractField(text, ['próximo passo', 'next step']), acceptanceCriteria: extractList(text, ['critérios de aceite(?:\\/|_)evidência', 'criterios de aceite(?:\\/|_)evidencia']).join(' · '), evidenceRef: sourcePath, createdAt: now, status: 'legacy', lastUpdate: now, lastBlocker: parsedBlockers[0]?.cause, blockers: parsedBlockers, sourceType: 'filesystem', source: sourcePath, historical: true, activeBlocker: active }
}
export function normalizeHandoff(input: Handoff, events: Event[] = []): Handoff {
  ...
  const status = input.status || (input.historical ? 'legacy' : active ? 'blocked' : 'received')
...
export async function syncHandoffs(state: FluxState, roots: string[] = [historyRoot, afterFortyPackage]): Promise<Handoff[]> {
  ...
  for (const base of roots) { let files: string[] = []; try { const walk = async (dir: string): Promise<void> => { for (const entry of await readdir(dir, { withFileTypes: true })) { const full = path.join(dir, entry.name); if (entry.isDirectory()) await walk(full); else if (/\.(md|yaml|yml)$/i.test(entry.name)) files.push(full) } }; await walk(base) } catch { continue }
    for (const sourcePath of files.sort()) { let text = ''; try { text = await readFile(sourcePath, 'utf8') } catch { continue }; text.split(/(?=\n\s*(?:[-*]\s*)?de\s*:)/i).forEach((chunk, index) => { const handoff = parseHistoricalHandoff(chunk, sourcePath, index); if (handoff && !found.has(handoff.id)) found.set(handoff.id, handoff) }) }
  }
  const current = (state.handoffs || []).map((h) => normalizeHandoff(h, state.events)); const unique = new Map([...current, ...found.values()].map((h) => [h.id, h])); state.handoffs = [...unique.values()].map((h) => normalizeHandoff(h, state.events)); return state.handoffs
}
...
async function load(file?: string): Promise<FluxState> {
  ...
  state = await syncArtifacts(state); await syncAgentRuns(state); await syncHandoffs(state)
...
export async function resumeHandoff(id: string, input: HandoffActionInput, actor: LocalActor, file?: string) { validateLocal(actor); if (!input.correlationId?.trim()) throw new FluxError('CORRELATION_REQUIRED', 'correlationId is required'); if (!input.cause?.trim() || !input.solution?.trim() || !input.nextAction?.trim() || !input.owner?.trim()) throw new FluxError('SOLUTION_REQUIRED', 'cause, solution, nextAction and owner are required', 422); const state = await load(file); const prior = state.events.find((e) => e.action === 'released handoff' && e.correlationId === input.correlationId); if (prior) return { handoff: state.handoffs.find((h) => h.id === id) || null, event: prior, idempotent: true }; const handoff = state.handoffs.find((h) => h.id === id); if (!handoff) throw new FluxError('HANDOFF_NOT_FOUND', `Handoff ${id} not found`, 404); if (handoff.status === 'legacy' || handoff.historical) throw new FluxError('LEGACY_HANDOFF', 'Legacy Handoffs are read-only', 409); const now = new Date().toISOString(); handoff.status = input.action === 'release' ? 'released' : 'in_progress'; handoff.lastUpdate = now; handoff.lastRelease = input.action === 'release' ? now : handoff.lastRelease; handoff.solution = { cause: input.cause, owner: input.owner, nextAction: input.nextAction, resolutionPlan: input.solution, resolutionEvidence: handoff.evidenceRef }; handoff.nextStep = input.nextAction; const event: Event = { id: `event-release-${input.correlationId}`, time: now, actor: actor.actor, action: 'released handoff', cardId: handoff.cardId, jobId: handoff.jobId, from: actor.actor, to: input.owner, correlationId: input.correlationId, reason: input.cause, solution: handoff.solution }; state.events.push(event); await save(state, file); return { handoff, event, idempotent: false } }
