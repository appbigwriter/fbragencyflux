...
export type ResolutionAction = { from: string; to: string; objective: string; deliverable: string; acceptanceCriteria: string; evidenceRequired: string; nextStep: string }
...
async function load(file?: string, importHistory = process.env.FLUX_IMPORT_HISTORY === '1'): Promise<FluxState> {
  ...
  state.handoffs = state.handoffs || []; state.events = state.events || []; state.approvals = state.approvals || []; state.requiredActions = state.requiredActions || []; state.coordinator = state.coordinator || {}
...
export async function forwardBlocker(blockerId: string, input: ForwardBlockerInput, actor: LocalActor, file?: string) {
  ...
  const requiredAction: RequiredActionRecord = { id: `required-action-forward-${input.correlationId}`, dedupeKey: `blocker-forward|${blockerId}|${input.correlationId}`, what: resolutionAction.nextStep, why: blocker.cause, who: resolutionAction.to, from: actor.actor, to: resolutionAction.to, objective: resolutionAction.objective, deliverable: resolutionAction.deliverable, acceptanceCriteria: resolutionAction.acceptanceCriteria, dueCheck: resolutionAction.nextStep, fallback: 'Escalar a Sergio se o owner não entregar a evidência.', status: 'hold', reasonCode: 'BLOCKER_OPEN', correlationId: input.correlationId, createdAt: now, updatedAt: now }
