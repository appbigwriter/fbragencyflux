...
export function configuredRepository(file?: string): FluxStateRepository { if (file) return new JsonFluxRepository(file); const mode = process.env.FLUX_PERSISTENCE || (process.env.NODE_ENV === 'production' ? 'supabase' : 'json'); const localFixture = process.env.FLUX_LOCAL_MODE === '1'; if (mode === 'json' && process.env.NODE_ENV === 'production' && !localFixture) throw new FluxError('JSON_PRODUCTION_DISABLED', 'JSON persistence is fixture/local only; configure Supabase persistence in production', 503); if (mode === 'json') { const testFile = process.env.FLUX_DATA_FILE; if (testFile) return new JsonFluxRepository(/* turbopackIgnore: true */ testFile); return new JsonFluxRepository(path.join(process.cwd(), 'data', 'flux-state.json')) }; if (mode !== 'supabase') throw new FluxError('PERSISTENCE_MODE_INVALID', `Unsupported FLUX_PERSISTENCE mode: ${mode}`, 500); const url = process.env.FLUX_SUPABASE_URL || process.env.SUPABASE_URL; const key = process.env.FLUX_SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY; if (!url || !key) throw new FluxError('PERSISTENCE_NOT_CONFIGURED', 'Supabase persistence requires runtime URL and service role key', 503); return new SupabaseFluxRepository(validateSupabaseUrl(url), key) }
