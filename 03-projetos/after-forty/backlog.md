# Backlog de Execução: After Forty 🌿

Acompanhamento direto e transparente dos entregáveis do projeto.

---

## 🏁 Fase 1: Fundação & Identidade
- [x] **Briefing e Definição de Escopo** (`brief.md`)
- [ ] **Mapeamento de Nicho & Palavras-Chave** (`01-pesquisa/keywords-e-concorrencia.md`)
- [ ] **Styleguide & Design Tokens** (`03-design-ui/tokens.css` / `styleguide.md`)
- [ ] **Character Bible & Assets da Heidi Braun** (`03-design-ui/assets/`)

---

## ✍️ Fase 2: Conteúdo Inicial (12 Artigos)
### Skin & Beauty
- [ ] Artigo 01: Retinol vs Bakuchiol for 40+ Skin (`02-conteudo/artigo-01.md`)
- [ ] Artigo 02: Essential Morning Skincare Routine for Mature Skin (`02-conteudo/artigo-02.md`)
- [ ] Artigo 03: Sunscreen Over 40: Mineral vs Chemical (`02-conteudo/artigo-03.md`)
- [ ] Artigo 04: Hyaluronic Acid & Ceramide Deep Hydration (`02-conteudo/artigo-04.md`)

### Recovery & Wellness
- [ ] Artigo 05: Sleep Optimization & Circadian Reset after 40 (`02-conteudo/artigo-05.md`)
- [ ] Artigo 06: Joint Health & Collagen: What Science Actually Says (`02-conteudo/artigo-06.md`)
- [ ] Artigo 07: Stress, Cortisol & Midlife Fatigue (`02-conteudo/artigo-07.md`)
- [ ] Artigo 08: Daily Mobility Routine for Lower Back & Hips (`02-conteudo/artigo-08.md`)

### Home Fitness
- [ ] Artigo 09: Creatine for Adults 40+: Benefits, Myths & Dosage (`02-conteudo/artigo-09.md`)
- [ ] Artigo 10: Building Muscle at 45: Resistance Bands & Dumbbells (`02-conteudo/artigo-10.md`)
- [ ] Artigo 11: Low-Impact Cardio for Heart Health & Fat Loss (`02-conteudo/artigo-11.md`)
- [ ] Artigo 12: Protein Intake Guide for Active Adults 40+ (`02-conteudo/artigo-12.md`)

---

## 💻 Fase 3: Desenvolvimento do Blog / Site
- [ ] **Setup da Aplicação Web (Next.js / Tailwind)** (`04-site/`)
- [ ] **Página Inicial (Hero, Artigos em Destaque, Newsletter, Sobre Heidi)**
- [ ] **Páginas de Categoria & Artigo Individual**
- [ ] **Componentes de Afiliados (Callout Box de Produtos)**
- [ ] **Páginas Institucionais (About, Contact, Privacy, Affiliate Disclosure)**

---

## 🚀 Fase 4: QA & Lançamento
- [ ] **Auditoria de SEO & Performance**
- [ ] **Preview Local Completo e Validação**
- [ ] **Gate de Deploy para Produção**
