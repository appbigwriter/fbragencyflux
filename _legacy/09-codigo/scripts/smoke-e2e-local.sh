...
ACTOR="Sergio"
SECRET="${FLUX_SMOKE_SECRET:?FLUX_SMOKE_SECRET must be provided; refusing to run with an embedded secret}"
...
docker run -d --name "$NAME" -p "${PORT}:3000" \
  -e NODE_ENV=production -e FLUX_LOCAL_MODE=1 -e FLUX_PERSISTENCE=json -e FLUX_DATA_FILE=/app/data/after-forty-intake.fixture.json -e FLUX_PUBLIC_READ_SCOPE=after-forty/* \
  -e FLUX_LOCAL_LOGIN_ACTOR="$ACTOR" -e FLUX_LOCAL_LOGIN_SECRET="$SECRET" "$IMAGE" >/dev/null
snapshot_code=""
for _ in $(seq 1 30); do snapshot_code=$(curl -sS "$BASE/api/flux/snapshot?scope=public&tenantId=after-forty" -o "$CURL_PAGE" -w '%{http_code}' 2>/dev/null || true); [ "$snapshot_code" = 200 ] && break; sleep 1; done
...
approval_code=$(curl -sS -b "$CURL_COOKIE" -H 'content-type: application/json' -d '{"decision":"approved"}' -o "$CURL_PAGE" -w '%{http_code}' "$BASE/api/flux/approvals/approval-af-intake-local/decision")
...
curl -fsS "$BASE/api/flux/snapshot?scope=public&tenantId=after-forty" > "$PAGE"
grep -q 'smoke handoff' "$PAGE"
docker restart "$NAME" >/dev/null
for _ in $(seq 1 30); do curl -fsS "$BASE/api/flux/snapshot?scope=public&tenantId=after-forty" > "$PAGE" 2>/dev/null && break; sleep 1; done
grep -q 'smoke handoff' "$PAGE"
