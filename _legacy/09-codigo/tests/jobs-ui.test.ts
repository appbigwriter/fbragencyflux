...
const job = (patch: Partial<AgentRun> = {}): AgentRun => ({
  ...
  artifactRefs: [], handoffRefs: [], evidenceRefs: [], blockers: [], nextStep: 'Revisar evidência', correlationId: 'corr-1', source: 'live/dispatcher', sourceType: 'live', historical: false, activeBlocker: false, startedAt: '2026-09-14T09:59:00Z', lastSeen: '2026-09-14T10:00:00Z', lastEvent: 'started', progress: 40, currentStep: 'Leitura', owner: 'Gabe', ...patch,
 })
...
describe('jobs operational view', () => {
  it('counts status, origin, live/historical, stale and blockers from real jobs', () => {
    ...
    expect(summary.byOrigin.filesystem).toBe(1); expect(summary.historical).toBe(1); expect(summary.live).toBe(1); expect(summary.stale).toBe(0); expect(summary.activeBlocker).toBe(1)
  })
  it('filters by project, card, agent, owner, origin, history, stale and blocker', () => {
    ...
    const filter: JobFilter = { project: 'FBR Agency Flux', card: 'FLUX-001', agent: 'Kora', owner: 'Sergio', origin: 'filesystem', lifecycle: 'historical', stale: false, blocker: true }
